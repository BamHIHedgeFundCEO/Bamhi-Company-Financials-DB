...
1. **判太寬那 27,219 格**（64.5%）—— 最大的一塊。方向是給 `build_evidence` 也加上
   行標題閘門：目前證據只看標籤詞元，所以 `OperatingLeaseLiabilityNoncurrent` 會被
   當成「這家有長期負債」的證據。用 plabel 是**獨立訊號**，不是把詞元規則調更嚴
   （那試過兩次都反效果），但一樣要逐一查證翻面結果才能上。
