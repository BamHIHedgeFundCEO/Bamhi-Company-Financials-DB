 ...
 *
 * 構面**按產業分七套**（通用／銀行／產險／壽險／權益型 REIT／資產管理／抵押型 REIT）。
 * 前六套按 SIC 選，抵押型 REIT 沒有自己的 SIC —— 它和權益型同住 6798，靠「有沒有
 * 投資性不動產」這個事實分流。頁面固定寫出
  ...
  <div>
    ...
    <main class="wrap sig">
      ...
      <template v-else-if="data">
        ...
        <section v-if="score" class="cardblock scoreblock">
          ...
          <p class="disclaimer">
            ...
            權益對資產差五倍）。
            <b>構面本身也分產業</b>：銀行、產險、壽險、權益型 REIT、資產管理、抵押型 REIT
            各有一套構面，通用模型的毛利率與存貨天數對它們沒有意義。前六套按 SIC 選；
            抵押型 REIT 和權益型同住 SIC 6798，改用「有沒有投資性不動產」這個事實分流
            —— 它不收租金、沒有物業，是一支用附買回融資的槓桿債券組合。
            評級級距與覆蓋率門檻七套共用，
      ...
      <p class="disclaim">
        ...
        跨產業必然有例外，所以逐項判讀一律把數值本身放在最前面、分級放在後面；
        §1 的總分是把這些項目加權整理，每一分都能在上面拆回原始數字與錨點。
